...
class Scan {
    ...
    var state: State {
        ...
        set {
            ...
            case .scanning where stateValue == .defineBoundingBox && !isReasonablySized,
                ...
                ScanObjectVC.instance?.showAlert(title: title, message: message, buttonTitle: "Yes", showCancel: true) { _ in
    ...
    private var isFirstScan: Bool {
        return ScanObjectVC.instance?.referenceObjectToMerge == nil
    }
    ...
    init(_ sceneView: ARSCNView) {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               name: ScanObjectVC.appStateChangedNotification,
                                               object: nil)
    ...
    private func applicationStateChanged(_ notification: Notification) {
        guard let appState = notification.userInfo?[ScanObjectVC.appStateUserInfoKey] as? ScanObjectVC.State else { return }
        switch appState {
        case .scanning:
    ...
    func updateOnEveryFrame(_ frame: ARFrame) {
        ...
        if state == .ready || state == .defineBoundingBox || state == .scanning {
            ...
            if let lightEstimate = frame.lightEstimate, lightEstimate.ambientIntensity < 500, !hasWarnedAboutLowLight, isFirstScan {
                ...
                ScanObjectVC.instance?.showAlert(title: title, message: message)
            }
        ...
        sceneView.session.createReferenceObject(
            ...
            completionHandler: { object, error in
                if let referenceObject = object {
                    ...
                    if let referenceObjectToMerge = ScanObjectVC.instance?.referenceObjectToMerge {
                        ScanObjectVC.instance?.referenceObjectToMerge = nil
                        
                        // Show activity indicator during the merge.
                        ScanObjectVC.instance?.showAlert(title: "", message: "Merging previous scan into this scan...", buttonTitle: nil)
                        ...
                        self.scannedReferenceObject?.mergeInBackground(with: referenceObjectToMerge, completion: { (mergedObject, error) in
                            ...
                            if let mergedObject = mergedObject {
                                ...
                                ScanObjectVC.instance?.showAlert(title: "Merge successful",
                                ...
                                ScanObjectVC.instance?.showAlert(title: "Merge failed", message: message, actions: [thisScan, previousScan])
                            }
                        })
                    } else {
                        creationFinished(self.scannedReferenceObject)
