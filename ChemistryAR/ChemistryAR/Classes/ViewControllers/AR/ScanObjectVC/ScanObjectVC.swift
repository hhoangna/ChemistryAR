...
class ScanObjectVC: BaseVC, ARSCNViewDelegate, ARSessionDelegate, UIDocumentPickerDelegate {
    ...
    @IBOutlet weak var instructionLabel: MessagesLabel!
    ...
    func createAndShareReferenceObject() {
        ...
        DispatchQueue.global().async {
            ...
            let airdropShareSheet = ShareScan(sourceView: self.nextButton, sharedObject: documentURL)
            DispatchQueue.main.async {
    ...
    fileprivate func mergeIntoCurrentScan(referenceObject: ARReferenceObject, from url: URL) {
        if self.state == .testing {
            ...
            ScanObjectVC.instance?.showAlert(title: "", message: "Merging other scan into this scan...", buttonTitle: nil)
